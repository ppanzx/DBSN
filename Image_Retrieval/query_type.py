...
def main():
    ...
    for qbname in qblists:
        ...
        v_hubness, t_hubness = Sinkhorn_Normalization(gq_t_sims, tau=0.01)
        sn_sims = q_t_sims-t_hubness[:,np.newaxis]

        t2i_r1 = sims2rank(sn_sims)
        t2i_r1s.append(t2i_r1)
        
        skewness = Skewness(sn_sims.T,10)
        logger.info("Skewness:%.3f"%(skewness))
        skewnesss.append(skewness)

        gq_t_emd = emd(gq_t_sims)
        logger.info("mean gq_t similarity:%.4f"%gq_t_emd)
        gq_t_emds.append(gq_t_emd)

        gq_q_sims = q_embs @ querybank[ids].T 
        gq_q_emd = emd(gq_q_sims)
        logger.info("mean gq_q similarity:%.4f"%gq_q_emd)
        gq_q_emds.append(gq_q_emd)

        logger.info("\n")

    logger.info(cap_lens)
    logger.info(t2i_r1s)
    logger.info(skewnesss)
    logger.info(gq_t_emds)
    logger.info(gq_q_emds)
